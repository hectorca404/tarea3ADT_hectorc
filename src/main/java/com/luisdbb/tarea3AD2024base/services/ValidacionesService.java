...
public class ValidacionesService {
	...
	public ValidacionesService(CredencialesRepository credencialesRepository, AlertsView alertsView, ParadasPeregrinosRepository paradasPeregrinosRepository, ParadaRepository paradaRepository) {
	...
	public Credenciales validarCredenciales(String nombreUsuario, String password) {
	    ...
	    if (usuarioAdmin != null && passAdmin != null 
	            && usuarioAdmin.equals(nombreUsuario) && passAdmin.equals(password)) {
	...
	public boolean validarCorreo(String correo) {
	 ...
	 public boolean validarNombreYApellido(String name, String apell) {
		    ...
		    for(int  i= 1; i< apell.length(); i++) {
		    	if(!Character.isLetter(apell.charAt(i))) {
		    		alertsView.mostrarError("Error", "El campo apellido no puede contener números ni caracteres especiales.");
		    		return false;
		    	}
		    }
	 ...
	 //SELLARALOJAR
	 public boolean yaHaSelladoHoy(Peregrino peregrino, Parada parada, LocalDate fecha) {
	 ...
	 public boolean validarNombreParadaYResponsable(String nombre) {
		 for(int i = 0; i<nombre.length(); i++) {
			 if(!Character.isLetter(nombre.charAt(i))) {
				 alertsView.mostrarError("Error", "El campo nombre de parada y el nombre del responsable no puede contener números ni caracteres especiales.");
				 return false;
			 }
		 }
		 return true;
